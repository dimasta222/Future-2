...
- src/components/constructor/constructorConfig.js
  ...
  - логика чтения этой конфигурации проходит через `resolveConstructorPrintArea(...)` и `resolveConstructorMockupSrc(...)`, поэтому новые мокапы можно подключать по одному размеру без правок UI и state-flow
