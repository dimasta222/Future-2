...
import { getConstructorShape, getConstructorShapeTightBounds, getConstructorTextFont, getConstructorTextGradient } from "../components/constructor/constructorConfig.js";
...
export default function useConstructorState({
  ...
  const getLayerMetrics = (layer, nextState = null) => {
    ...
    if (resolvedLayer.type === "shape") {
      const widthCm = resolvedLayer.widthCm;
      const heightCm = resolvedLayer.heightCm ?? widthCm;
      const baseWidth = Math.min(areaWidth, areaWidth * (widthCm / areaWidthCm));
      const baseHeight = Math.min(areaHeight, areaHeight * (heightCm / areaHeightCm));
      ...
      return {
        areaWidth,
        areaHeight,
        width: frameMetrics.frameWidthPx,
        height: frameMetrics.frameHeightPx,
        baseWidth,
        baseHeight,
        frameMetrics,
      };
  ...
  const handleLayerPointerDown = (layerId, event) => {
    ...
    const startGroupLeft = Math.min(...movingLayers.map((item) => item.startCenterXPx - (item.metrics.width / 2)));
    const startGroupTop = Math.min(...movingLayers.map((item) => item.startCenterYPx - (item.metrics.height / 2)));
    const startGroupRight = Math.max(...movingLayers.map((item) => item.startCenterXPx + (item.metrics.width / 2)));
    const startGroupBottom = Math.max(...movingLayers.map((item) => item.startCenterYPx + (item.metrics.height / 2)));
  ...
  const normalizeTextLayerState = (candidateLayer, previousLayer = candidateLayer) => {
    ...
    nextLayer.textBoxWidth = Math.min(100, Math.max(MIN_TEXT_BOX_WIDTH_PERCENT, Number(nextLayer.textBoxWidth ?? previousLayer.textBoxWidth ?? 88)));
  ...
  const setShapeKey = (nextShapeKey) => {
    ...
    updateActiveShapeLayer((layer) => ({
      ...layer,
      shapeKey: resolvedShapeKey,
      ...getShapeDimensionsFromWidthCm(resolvedShapeKey, layer.widthCm ?? activeShapeLayer.widthCm ?? 12, getLayerSide(layer)),
    }));
  };
  ...
  const setShapeWidthCm = (nextWidthCm) => {
    if (!activeShapeLayer) return;
    pushHistoryCheckpoint();
    const nextDimensions = getShapeDimensionsFromWidthCm(activeShapeLayer.shapeKey, nextWidthCm, getLayerSide(activeShapeLayer));
    updateLayer(activeShapeLayer.id, (layer) => ({
      ...layer,
      ...nextDimensions,
      position: clampLayerPosition(layer.position, { ...layer, ...nextDimensions }, getLayerMetrics(layer, nextDimensions)),
