...
export function resizeShapeLayer({
  ...
  if (isCornerHandle(handle)) {
    ...
    const requestedBaseSize = solveBaseSizeFromOuterSize(
      layer,
    ...
    const widthMultiplier = requestedBaseSize.baseWidthPx / startBaseWidthPx;
    const heightMultiplier = requestedBaseSize.baseHeightPx / startBaseHeightPx;
    const uniformMultiplier = Math.max(
      Math.max(MIN_SHAPE_WIDTH_PX / startBaseWidthPx, MIN_SHAPE_HEIGHT_PX / startBaseHeightPx),
      Math.abs(widthMultiplier - 1) >= Math.abs(heightMultiplier - 1) ? widthMultiplier : heightMultiplier,
    );
