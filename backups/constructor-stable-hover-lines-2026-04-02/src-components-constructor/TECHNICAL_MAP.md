...
- physical print model for oversize black XS/S mockups: front/back PNG plus print-area 40 × 50 см; upload, preset и обычные shape-layer хранят widthCm/heightCm, а line-shape дополнительно хранит lineWidthPx/lineHeightPx в логических px холста и конвертирует factual size в см на лету
...
- upload/preset/regular-shape rendering via layer widthCm/heightCm mapped into physical print-area; line-shape rendering via lineWidthPx/lineHeightPx mapped into ту же print-area как logical canvas pixels
...
- active layer resize handles with proportional corner scaling and one-axis edge stretching for non-text layers, при этом corner-resize для upload/preset/shape остаётся uniform, у shape-layer противоположный угол служит опорной точкой для углового resize, а боковые маркеры фиксируют противоположную сторону внешней рамки и меняют базовую геометрию вместе с tight bounds
