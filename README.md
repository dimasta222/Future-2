...
## Архитектурные правила конструктора

- Единый документ с контрактами по слоям и resize: [ARCHITECTURE_RULES.md](ARCHITECTURE_RULES.md)
...
### Одна команда для публикации

Если хотите сразу сделать всё за один раз — подтянуть изменения, собрать проект, создать commit и отправить его в GitHub — используйте:

```bash
npm run publish -- "описание изменений"
```

Что делает команда:

1. выполняет `git pull --rebase`
2. запускает `npm run build`
3. добавляет все изменения в commit
4. отправляет commit в GitHub

После этого GitHub Actions автоматически обновит опубликованный сайт.
