...
- resize активного слоя прямо на превью: для текста боковые handles меняют ширину контейнера и перенос, угловые масштабируют контейнер и размер текста как единый объект
- настройку ширины text box и автопереноса через resize на превью
- показ размеров текста и контейнера в сантиметрах в sidebar на основе реального DOM-измерения внутри physical print-area
