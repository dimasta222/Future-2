...
export async function exportPrintPdf({ layers, printArea }) {
  ...
  console.log("[exportPrintPdf] start", { physW, physH, pageW, pageH, layerCount: layers.length });
...
async function renderTextLayer(doc, layer, printArea, fontCache) {
  ...
  // Try vector rendering with TTF for simple text (no effects)
  if (!needsCanvasRender) {
    const registeredFont = await loadAndRegisterFont(doc, fontCache, font.label, fontWeight);
    if (registeredFont) {
      try {
        const fontSizePt = cmToPt(fontSizeCm);
        doc.setFont(registeredFont, "normal");
        ...
        if (rotationDeg) {
          ...
          const sin = Math.sin(rad);
          doc.setCurrentTransformationMatrix(
            doc.Matrix(cos, sin, -sin, cos, xPt - xPt * cos + yPt * sin, yPt - xPt * sin - yPt * cos),
          );
        }
        ...
        console.warn("[exportPrintPdf] TTF vector text failed, falling back to canvas:", vectorErr.message);
      }
    }
  }
...
async function renderShapeLayer(doc, layer, printArea) {
  ...
  try {
    ...
    const drawXCm = pos.cx - pos.w / 2 - padLeftCm;
    const drawYCm = pos.cy - pos.h / 2 - padTopCm;
