...
- resize активного слоя прямо на превью: для текста боковые handles меняют ширину контейнера и перенос, угловые масштабируют контейнер и размер текста как единый объект, для upload/preset/shape угловой resize остаётся uniform
