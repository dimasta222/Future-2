...
export default function ConstructorPreviewPanel({
  ...
  const getShapeRenderFrame = (layer) => {
    ...
    const baseWidthPx = ((layer.widthCm || 1) / physicalWidthCm) * printAreaWidthPx;
    const baseHeightPx = ((layer.heightCm || 1) / physicalHeightCm) * printAreaHeightPx;
  ...
  const handleLayerResizePointerDown = (layer, handle, event) => {
    ...
    const dragState = {
      ...
      startBoundsRight,
      startBoundsBottom,
      shapeFrameMetrics: layer.type === "shape"
        ? getShapeFrameMetricsPx(layer, {
          baseWidthPx: ((layer.widthCm || 1) / physicalWidthCm) * printAreaBounds.width,
          baseHeightPx: ((layer.heightCm || 1) / physicalHeightCm) * printAreaBounds.height,
        })
        : null,
    };
