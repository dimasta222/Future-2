...
- **Пропорции:** Каждый тип товара имеет `mockupAspectRatio` (из constructorConfig) для корректного преобразования физических размеров в PDF-координаты — вычисляется `effectivePhysH = physW * (areaH/areaW) / mockupAR`
