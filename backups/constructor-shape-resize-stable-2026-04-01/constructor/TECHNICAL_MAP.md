...
- SVG shape-layer rendering with tight SVG bounds and effect-aware outer frame, чтобы тень, искажение и обводка не обрезались preview-box'ом
- active layer resize handles with proportional corner scaling and one-axis edge stretching for non-text layers, при этом corner-resize для upload/preset/shape остаётся uniform, у shape-layer противоположный угол служит fixed anchor для углов, а боковые handles фиксируют противоположную сторону outer frame и меняют base geometry вместе с tight bounds
