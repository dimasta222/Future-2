...
- resize активного слоя прямо на превью: для текста боковые handles меняют ширину контейнера и перенос, угловые масштабируют контейнер и размер текста как единый объект, для upload/preset/shape угловой resize остаётся uniform, а у shape-layer угловой resize держит fixed anchor в противоположном углу, а боковой resize фиксирует противоположную сторону и деформирует саму фигуру вместе с tight frame
...
- effect-aware рамку shape-layer: preview-box расширяется под тень, искажение и обводку и остаётся вплотную к реальной фигуре без обрезания эффекта
- загрузку файла
